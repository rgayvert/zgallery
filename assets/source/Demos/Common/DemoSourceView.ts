import { zutil, core, atom, pct, css_length, em, createToggleAtom, ToastButton } from "zaffre";
...
export function DemoSourceView(sourceDir: string, fileNames: string[], maxHeight?: css_length): View {
  ...
  // Option to strip comments, adapted from https://stackoverflow.com/questions/21817453 and https://stackoverflow.com/questions/37051797
  ...
  function stripCommentsAndHighlight(source: string): string {
    return highlightSourceText(reduceMultipleBlankLines(source.replace(/\/\*[\s\S]*?\*\/|(?<=[^:])\/\/.*|^\/\/.*/g, "")));
  }
  ...
  const showComments = createToggleAtom(false);
