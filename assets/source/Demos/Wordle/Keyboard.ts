...
import { atom, standardHTMLInteractionEffects, em, vmin, createFlexToken } from "zaffre";
...
export function Keyboard(options: KeyboardOptions = {}): View {
  ...
  function keyButton(key: string): View {
    return Button({
      label: key,
      ...
      flex: key.length === 1 ? createFlexToken({ grow: 1, shrink: 1 }) : undefined,
      minWidth: em(1.5),
  ...
  return VStack({ gap: core.space.s3 }).append(
    ...keys.map((_row, index) => HStack({ gap: core.space.s3 }).append(...keys[index].map((key) => keyButton(key))))
  );
}
