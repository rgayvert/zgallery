...
export function ImageFiltersExample(): View {
  ...
  function slider(spec: FilterSpec): View {
    return Slider(spec.value, { minVal: 0, maxVal: spec.max, width: ch(10), alignSelf: "center" });
  }
  return HStack({ gap: core.space.s5 }).append(
    VStack({
      ...
      Grid({ ncolumns: 2, nrows: 9, gridAutoFlow: "column", columnGap: ch(1), rowGap: core.space.s2 }).append(
        ...model.filters.map((spec) => TextLabel(spec.title)),
        ...model.filters.map((spec) => slider(spec))
      )
    ),
